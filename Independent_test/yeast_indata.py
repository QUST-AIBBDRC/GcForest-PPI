...
X_shu=scaler.transform(X_shuffle) 
train_dim=X_shu[:,mask]
train_shu=np.reshape(train_dim,(train_dim.shape[0],train_dim.shape[2]))
train_label=y


data_test= sio.loadmat('Ecoli_feature_end.mat')
...
train_shu= train_shu[:, np.newaxis,np.newaxis,:]
test_shu=test_shu[:, np.newaxis,np.newaxis,:]
gc = GCForest(config)
hist=gc.fit_transform(train_shu, train_label)
y_score=gc.predict_proba(test_shu)
with open("model_gc4.pkl","wb") as f:
    pickle.dump(gc,f,pickle.HIGHEST_PROTOCOL)
y_test=utils.to_categorical(test_label)    
y_class= utils.categorical_probas_to_classes(y_score)
sio.savemat('Celeg_class.mat', {'Celeg_class':y_class})
