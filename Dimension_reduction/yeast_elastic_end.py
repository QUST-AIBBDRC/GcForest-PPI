...
start=time.time()
path1='gcforest4.json.json'

config = load_json(path1)
gc = GCForest(config)
extraction= sio.loadmat('yeast_feature.mat')
...
X_dim_,mask=elasticNet(X_shuffle,y,alpha =0.03,l1_ratio=0.1)
...
yscore_sum.to_csv('yeast_elastic_yscore.csv')
ytest=ytest[np.array(range(1,row)),:]
ytest_sum = pd.DataFrame(data=ytest)
ytest_sum.to_csv('yeast_elastic_ytest.csv')
...
data_csv.to_csv('yeast_elastic_result.csv')
sio.savemat('yeast_elastic_mask_scale_0.03_0.1.mat', {'yeast_elastic_mask':mask})
